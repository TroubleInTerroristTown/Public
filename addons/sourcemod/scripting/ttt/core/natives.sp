...
void InitNatives()
{
	...
	CreateNative("TTT_GetRoleNameByID", Native_GetRoleNameByID);
	CreateNative("TTT_GetRoleNameByShortName", Native_GetRoleNameByShortName);
	CreateNative("TTT_GetRoleIDByName", Native_GetRoleIDByName);
...
public int Native_GetRoleNameByID(Handle plugin, int numParams)
{
	int role = GetNativeCell(1);

	if (role == TTT_TEAM_INNOCENT)
	{
		SetNativeString(2, "Innocent", GetNativeCell(3));
	}
	else if (role == TTT_TEAM_DETECTIVE)
	{
		SetNativeString(2, "Detective", GetNativeCell(3));
	}
	else if (role == TTT_TEAM_TRAITOR)
	{
		SetNativeString(2, "Traitor", GetNativeCell(3));
	}
	else
	{
		SetNativeString(2, "Unassigned", GetNativeCell(3));
	}
}

public int Native_GetRoleNameByShortName(Handle plugin, int numParams)
{
	char sRole[ROLE_LENGTH];
	GetNativeString(1, sRole, sizeof(sRole));

	if (StrEqual(sRole, "inno", false))
	{
		SetNativeString(2, "Innocent", GetNativeCell(3));
	}
	else if (StrEqual(sRole, "dete", false))
	{
		SetNativeString(2, "Detective", GetNativeCell(3));
	}
	else if (StrEqual(sRole, "trai", false))
	{
		SetNativeString(2, "Traitor", GetNativeCell(3));
	}
	else
...
public int Native_GetRoleIDByName(Handle plugin, int numParams)
{
	char sRole[ROLE_LENGTH];
	GetNativeString(1, sRole, sizeof(sRole));

	if (StrEqual(sRole, "innocent", false))
	{
		return TTT_TEAM_INNOCENT;
	}
	else if (StrEqual(sRole, "detective", false))
	{
		return TTT_TEAM_DETECTIVE;
	}
	else if (StrEqual(sRole, "traitor", false))
	{
...
public int Native_GetRoleIDByShortName(Handle plugin, int numParams)
{
	char sRole[ROLE_LENGTH];
	GetNativeString(1, sRole, sizeof(sRole));
...
public int Native_GetShortRoleNameByID(Handle plugin, int numParams)
{
	int role = GetNativeCell(1);
	
	if (role == TTT_TEAM_INNOCENT)
	{
		SetNativeString(2, "inno", GetNativeCell(3));
	}
	else if (role == TTT_TEAM_DETECTIVE)
	{
		SetNativeString(2, "dete", GetNativeCell(3));
	}
	else if (role == TTT_TEAM_TRAITOR)
	{
		SetNativeString(2, "trai", GetNativeCell(3));
	}
	else
	{
		SetNativeString(2, "unas", GetNativeCell(3));
	}
}

public int Native_GetShortRoleNameByName(Handle plugin, int numParams)
{
	char sRole[ROLE_LENGTH];
	GetNativeString(1, sRole, sizeof(sRole));
