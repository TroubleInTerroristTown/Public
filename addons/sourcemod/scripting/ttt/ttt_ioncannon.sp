...
public void OnPluginStart()
{
	...
	g_cIonPrefix = AutoExecConfig_CreateConVar("ion_cannon_prefix", "{green}Ion Cannon {default}»", "TTT Ion cannon prefix.");
...
{
	g_cIonPrefix.AddChangeHook(OnConVarChanged);
	g_cIonPrefix.GetString(g_sPluginTag, sizeof(g_sPluginTag));
...
{
    if (convar == g_cIonPrefix)
    {
        g_cIonPrefix.GetString(g_sPluginTag, sizeof(g_sPluginTag));
    }
	else if (convar == g_cIonDeployWeapon)
    {
        g_cIonDeployWeapon.GetString(g_sIonWeapon, sizeof(g_sIonWeapon));
    }
}
